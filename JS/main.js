...
lObject2["rule3"] = "";
lObject2["length"] = 20;
lObject2["canvasHeight"] = window.innerHeight/2 - 200;
...
lObject3["length"] = -10;
lObject3["canvasHeight"] = window.innerHeight - 200;
...
function generateSequence(){
    ...
    if(document.getElementById('iterator').value != "")
...
function drawStuff() {
        context.translate(canvas.width/2, canvas.height - lObject.canvasHeight);
        context.moveTo(0,0);
        if(document.getElementById('iterator').value != "")
            lObject.round = parseInt(document.getElementById('iterator').value); 
        var r = (lObject.length + 30 ) / lObject.round;
        var lastPosX = 0;
        var lastPosY = 0;
